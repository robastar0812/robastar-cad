...
// ── バルーン矢印INSERT座標キャッシュ ──
// _OPEN系・矢印系ブロックのINSERT挿入点を先端座標として登録
function buildArrowTips() {
  if(!S.f1 || !S.f1.parsed) return [];
  const allEnts = S.f1.parsed.entities;
  // 元のINSERTエンティティはexpandInsert後に entities には残らない
  // → パース前の rawInserts をキャッシュする必要があるため
  //    ここでは TEXT エンティティの座標周辺にある LINE の端点を
  //    バルーン矢印先端候補として返す
  return [];
}
...
function getLayerColor(name, aciColor){
  if(!S.layers[name]){
    ...
    const col = resolvedAci || LAYER_COLORS[layerColorIdx%LAYER_COLORS.length];
    // 図枠系レイヤーはデフォルト非表示
    const visible = !FRAME_LAYER_PAT.test(name);
    S.layers[name]={visible,color:col,count:0};
    if(!resolvedAci) layerColorIdx++;
  }
...
function parseDXF(text){
  ...
  // 矢印系ブロックのINSERT挿入点を収集（引き込み線先端検出用）
  const arrowTips = [];
  // ENTITIES セクションを再スキャンしてINSERT挿入点を取得
  for(let i=es;i<ee;i++){
    const[c,v]=pairs[i];
    if(c===0&&v==='INSERT'){
      const d=readEntPairs(pairs,i,ee,40);
      const bname=(d[2]||'').toUpperCase();
      // _OPEN系・矢印系ブロック名を判定
      if(/^(_OPEN|ARROW|BALLOON|LEADER|MKARROW|DOT)/i.test(bname)){
        const tx=parseFloat(d[10])||0;
        const ty=parseFloat(d[20])||0;
        arrowTips.push({x:tx,y:ty,blockName:d[2]||''});
      }
    }
  }
  return{entities,layers:Object.keys(layerMap),layerCounts:layerMap,aciMap:layerAci,styleMap,arrowTips};
}
...
function buildPartGroups() {
  ...
  textEnts.forEach(e => {
    ...
    // コロン以降をコメントとして分離（例: A910C:COIL1,2 → A910C）
    const key = raw.split(':')[0].trim();
    if(!balloonPat.test(key)) return;
    if(!groups[key]) groups[key] = { texts:[], lines:[], comments:[] };
    groups[key].texts.push(e);
    // コロン以降があればコメントとして保存
    const comment = raw.includes(':') ? raw.slice(raw.indexOf(':')+1).trim() : null;
    if(comment) groups[key].comments.push(comment);
  });
...
function findNearbyTexts(e, maxDist) {
  ...
  if(isLine){
    ...
    // arrowTips（_OPEN30等の矢印INSERT挿入点）が近ければ追加
    const tips = S.f1&&S.f1.parsed&&S.f1.parsed.arrowTips||[];
    tips.forEach((tip,ti)=>{
      // チェーン内のいずれかの端点から searchR 以内の矢印先端を追加
      for(const [px,py] of allPts.values()){
        if(Math.hypot(px-tip.x,py-tip.y)<=searchR){
          allPts.set(`_tip_${ti}`,[tip.x,tip.y]);
          break;
        }
      }
    });
